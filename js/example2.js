function MenuChoice(selection)
{
    ...
    switch (selection)
    {
        case "storelist":document.getElementById("storelist").style.visibility = "visible"; //Makes the Store List HTML section visible
        ListStores(); //Calls the function that creates the store list
        break;
        case "orderhistory":document.getElementById("orderhistory").style.visibility = "visible";
        break;
        case "None"://No menu item selected, so no section should be displayed
        break;default:alert("Please select a different menu option");
        }
        }
...
function ListStores() //This sends a request to the GetAllStores service and creates a table with the data returned
{
    ...
    var url = "https://student.business.uab.edu/WebAppService/service1.svc/getAllStores"; //URL for the service
    xmlhttp.onreadystatechange = function() { //Creates the event handler for service request
        
        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {var output = JSON.parse(xmlhttp.responseText); //Captures the data returned form the service and puts in an object
...
function GenerateOutput(result) //This function receives the data form the service and creates a table to display it
{
    ...
    { //Anchor link: <a href="javascript:function("parameter");">
        ...
        storename = '<a href="javascript:Orders(' + "'" + storeid + "');" + '">';
        storename += result.GetAllStoresResult[count].StoreName;storename += '</a>';
    ...
    {
        var xmlhttp = new XMLHttpRequest();var url = "https://student.business.uab.edu/WebAppService/service1.svc/getOrderHistory/"; //Service URL
        ...
        xmlhttp.onreadystatechange = function() {if (xmlhttp.readyState == 4 && xmlhttp.status == 200)
        {
            ...
            function GenerateOutput(result) //Function that displays results
            {
                ...
                var count = 0;for(count = 0; count < result.length; count ++)
                {
                    ...
                    display += "</table>";document.getElementById("books").innerHTML = display;
